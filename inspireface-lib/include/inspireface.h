...
#define HF_ENABLE_DETECT_MODE_FACE_LANDMARK 0x00000200  ///< Flag to enable face landmark detection feature.
 ...
 * @return HResult indicating the success or failure of the operation.
 */
HYPER_CAPI_EXPORT extern HResult HFImageBitmapDrawRect(HFImageBitmap handle, HFaceRect rect, HFColor color, HInt32 thickness);
 ...
 * @return HResult indicating the success or failure of the operation.
 */
HYPER_CAPI_EXPORT extern HResult HFImageBitmapDrawCircle(HFImageBitmap handle, HPoint2i point, HInt32 radius, HFColor color, HInt32 thickness);
...
HYPER_CAPI_EXPORT extern HResult HFInspireFaceConfiguringIOSExtensionCoreMLPackagePath(HPath coremlPackage);
 ...
 *          the need to input a multiple of 160, such as 160, 192, 256, 320, 640, the default value -1 is 160.
...
/**
 * @brief Set the track mode smooth ratio in the session. default value is 0.06
 *
 * @param session Handle to the session.
...
typedef struct HFFaceFeature {
    ...
    HFloat norm;            ///< Norm of the feature data().
                            // Note: norm is only valid if the 'HFFaceFeatureExtract' interface is used as the return value
 ...
 * @return HResult indicating the success or failure of the operation.
 */
HYPER_CAPI_EXPORT extern HResult HFFeatureHubGetFaceIdentity(HInt32 id, PHFFaceFeatureIdentity identity);
